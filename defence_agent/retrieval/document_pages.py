...
class DocumentPage:
    """One rendered source page ready for multimodal embedding."""
...
def load_document_pages(corpus_dir: Path | None = None) -> list[DocumentPage]:
    ...
    cached_pages = _load_manifest_pages_cached(str(manifest_path.resolve()), stat.st_mtime_ns, stat.st_size)
...
def _load_manifest_pages_cached(
    ...
    _ = (manifest_mtime_ns, manifest_size)
...
def _load_manifest_pages(manifest_path: Path) -> list[DocumentPage]:
    ...
    for entry in documents:
        ...
        pdf_path = _resolve_manifest_pdf(manifest_path.parent, entry)
        pages.extend(_pages_from_pdf(pdf_path, metadata, manifest_path=manifest_path))
        if not pages or pages[-1].doc_id != doc_id:
            raise ValueError(f"Manifest entry {doc_id} produced no pages")
    return pages


def _resolve_manifest_pdf(corpus_root: Path, entry: dict[str, Any]) -> Path:
    raw_path = entry.get("path") or entry.get("pdf_path")
    if not raw_path:
        raise ValueError("Manifest document entry is missing path")
    pdf_path = Path(str(raw_path)).expanduser()
    if not pdf_path.is_absolute():
        pdf_path = corpus_root / pdf_path
    if not pdf_path.exists():
        raise FileNotFoundError(f"Manifest PDF does not exist: {pdf_path}")
    return pdf_path
...
def _manifest_metadata(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in entry.items():
        if key in {"path", "pdf_path", "metadata"}:
            continue
